...
def faultnetwork3D(hypo_file, hypo_sep, out_dir, n_mc, r_nn, dt_nn, validation_bool, 
        ...
        for i in range(it):
            ...
            # Directional statistics for all events with more than 80% fits
                ...
                try:
                    ...
                    mean_vector = np.array([np.nanmean(nor_x), np.nanmean(nor_y), np.nanmean(nor_z)])
                    kappa = -999
                    beta = -999
    ...
    max_kappa = np.nanmax(data_output['kappa'])
    data_output['kappa'] = data_output['kappa'].replace(-999, max_kappa)
